...
## ADR-015: Staff invite activation — signed JWT link + separate OTP second factor

**Context:** [16-AUTHENTICATION.md](16-AUTHENTICATION.md)'s original "Staff activation" section (Stage 1) specified the shape in one under-specified sentence: an emailed "signed, time-limited (72h) activation link" that is "OTP-verified against their email as a second factor," landing on a single `POST /users/activate` endpoint. Implementing it in Phase 4 required resolving two things the original text left open: what the "signed" link actually is, and how a second-factor OTP fits alongside a single endpoint when the OTP mechanism elsewhere in this system (`OtpChallenge`) always needs its own creation step before it can be verified.

**Decision:** The activation link carries an **RS256-signed JWT** (`ActivationTokenService`, `apps/api/src/common/jwt/`), reusing the same keypair as access tokens but with a `typ: "activation"` claim so the two token kinds can never be accepted in place of each other, and a 72h expiry. Completing activation is **two calls**, matching the already-established `/auth/forgot-password` → `/auth/reset-password` shape rather than inventing a new protocol: `POST /users/activate/request-otp` (new route) creates/resends an `OtpChallenge` under a new `ACCOUNT_ACTIVATION` purpose (its own enum value, never confusable with `REGISTRATION`/`PASSWORD_RESET`); `POST /users/activate` then verifies that code — cross-checked against the activation token's own `sub` so a challenge can never be replayed against a different invitee's account — sets the password, and activates the user.

**Consequences:** Reusing an existing, already-tested two-call shape (rather than a novel single-endpoint design) kept the implementation small and consistent, at the cost of one more network round-trip than the original one-sentence spec implied. The activation token and the OTP are genuinely independent factors: the token proves possession of the emailed link (which could be replayed from browser history/logs up to 72h later), the OTP proves live control of the inbox *right now* — collapsing them into one factor would have been a real, if subtle, security regression from what Stage 1 asked for. [15-API-SPECIFICATION.md](15-API-SPECIFICATION.md) and [16-AUTHENTICATION.md](16-AUTHENTICATION.md) were updated in the same change that implemented this, per the standing rule that an implementation decision not fully covered by docs gets resolved in the docs, not left to silently diverge.
