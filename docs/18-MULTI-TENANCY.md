...
`appointments.read` (Phase 7) follows the same shape as `patients.read`: `PATIENT` is `SELF` (own `Appointment.patientId`), `DOCTOR` is `ASSIGNED` (own `Appointment.doctorId` — a Doctor sees exactly the appointments where they are the treating doctor, nothing broader), `NURSE`/`RECEPTIONIST` are `BRANCH` (own `Staff.branchId`, resolved identically to `doctors.read`'s branch scope), `ADMIN` is `HOSPITAL`, `SUPER_ADMIN` is `PLATFORM`. `apps/api/src/appointments/appointment-access.util.ts`'s `loadAppointmentInScope()` is the single shared enforcement point, reused as the prerequisite check before every mutation (reschedule/cancel/checkin/no-show) — a caller must first be able to *read* an appointment before any sub-action rule is even evaluated. `appointments.update` conflates two different sub-actions behind one permission string (docs/02-PERSONAS-AND-ROLES.md's per-role prose): reschedule (`PATIENT` self, `RECEPTIONIST`/`ADMIN`/`SUPER_ADMIN` at their normal scope) and no-show (`DOCTOR`, `RECEPTIONIST`/`ADMIN`/`SUPER_ADMIN` — never `PATIENT`) — `apps/api/src/appointments/appointment-transitions.service.ts` enforces this split at the service layer on top of the guard's single permission check, exactly the same pattern as `schedules.read`'s availability/template split above. `appointments.checkin` is `SELF` for both `PATIENT` and `DOCTOR` (each checking in their own side of the same appointment), `BRANCH` for `NURSE`/`RECEPTIONIST`, `HOSPITAL`/`PLATFORM` for `ADMIN`/`SUPER_ADMIN` — the same `loadAppointmentInScope()` gate covers it, no separate check needed.

`Patient.registeredHospitalId`/`registeredBranchId` (Phase 5) are a narrower, separate concept — which hospital's front desk originally registered them, not where their clinical data lives. This is what a Receptionist's (`BRANCH` scope) or Admin's (`HOSPITAL` scope) `GET /patients` list filters by; a Doctor's `patients.read` (`ASSIGNED` scope) does **not** use it at all. Phase 7's `Appointment` model now exists and is exactly the "actual appointment/consultation relationship" this scope was always meant to key off — but `PatientsService.resolveScope()` has not yet been updated to use it (still returns no rows for a Doctor caller, a deliberate scope decision carried forward rather than expanded mid-phase; see docs/42-PROJECT-STATE.md's Known Issues). A patient who has only ever self-registered via the mobile app has both fields `null` and is invisible to every hospital's staff-facing patient list until some future encounter associates them with one — expected, not a bug: nothing yet has "claimed" them.
